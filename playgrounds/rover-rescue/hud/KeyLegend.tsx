...
export function KeyLegend() {
  return (
    <div
      ...
      role="dialog"
      aria-label="Map key"
      className="absolute bottom-2 right-[8.5rem] z-30 w-52 rounded-lg border border-white/30 bg-slate-950/90 p-3 text-white shadow-xl backdrop-blur-md"
      onMouseDown={(event) => event.stopPropagation()}
    >
      <div className="mb-2 text-[11px] font-bold uppercase tracking-wider text-violet-200">Map key</div>
      <div className="grid grid-cols-2 gap-x-3 gap-y-2 text-[10px]">
        <KeyItem icon={<Package />} color="text-cyan-300" label="Minerals" />
        <KeyItem icon={<Bot />} color="text-sky-300" label="Rover" />
        <KeyItem icon={<Mountain />} color="text-stone-300" label="Rock" />
        <KeyItem icon={<Flower2 />} color="text-yellow-400" label="Plant" />
        ...
        <KeyItem icon={<Biohazard />} color="text-lime-400" label="River hazard" />
        <KeyItem icon={<Route />} color="text-stone-300" label="Bridge" />
